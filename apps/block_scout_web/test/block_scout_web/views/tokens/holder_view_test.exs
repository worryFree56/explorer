defmodule BlockScoutWeb.Tokens.HolderViewTest do
  ...
  describe "format_token_balance_value/3" do
    test "formats according to token decimals when it's a PRC-20" do
      token = build(:token, type: "PRC-20", decimals: Decimal.new(2))
    ...
    test "returns the value when it's PRC-721" do
      token = build(:token, type: "PRC-721")
