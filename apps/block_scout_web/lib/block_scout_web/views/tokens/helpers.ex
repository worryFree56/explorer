defmodule BlockScoutWeb.Tokens.Helpers do
  ...
  defp do_token_transfer_amount(%Token{type: "PRC-20"}, nil, nil, _token_id, _token_ids) do
    {:ok, "--"}
  end

  defp do_token_transfer_amount(%Token{type: "PRC-20", decimals: nil}, amount, _amounts, _token_id, _token_ids) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, Decimal.new(0))}
  end

  defp do_token_transfer_amount(%Token{type: "PRC-20", decimals: decimals}, amount, _amounts, _token_id, _token_ids) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, decimals)}
  end

  defp do_token_transfer_amount(%Token{type: "PRC-721"}, _amount, _amounts, _token_id, _token_ids) do
    {:ok, :erc721_instance}
  end

  defp do_token_transfer_amount(%Token{type: "PRC-1155", decimals: decimals}, amount, amounts, _token_id, token_ids) do
    if amount do
